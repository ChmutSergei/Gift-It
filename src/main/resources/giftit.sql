...
CREATE TABLE Carts(id BIGINT PRIMARY KEY AUTO_INCREMENT, user_id BIGINT, item_id BIGINT, count DECIMAL,
  FOREIGN KEY (user_id) REFERENCES Users(id) ON DELETE  CASCADE  ON UPDATE CASCADE,
  FOREIGN KEY (item_id) REFERENCES Items(id) ON DELETE  CASCADE  ON UPDATE CASCADE);
CREATE TABLE Orders(id BIGINT PRIMARY KEY AUTO_INCREMENT, user_id BIGINT, cart_id BIGINT,
